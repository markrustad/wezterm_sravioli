...
- The background doesn't change via backdrop picker

- The background dissapears when the coloscheme picker is run (and will not return until the next WezTerm launch)
...
```lua
    ...
    -- wezterm.log_info("DEBUG keystroke dump: set debug_key_events=true")
    -- config.debug_key_events = true
    -- config.warn_about_missing_glyphs = false

    -- make UCRT64 the default on Windows (as requested)
    ...
    config.default_cwd = "C:\\Users\\rustadmd\\msys64\\home\\rustadmd"
    else
    config.default_prog = { "zsh", "-l" }
    end

    -- Initialize backdrop functionality from the downloaded plugin
...
Also, each color scheme in the `picker/assets/colorschemes` explicitly sets the background color to `colors.background`
